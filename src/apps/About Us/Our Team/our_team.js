...
class OurTeamPage extends Component{
    render(){
        ...
        var section_1_list = [
            {
                title: "Why Choose Us",
                content: "We are committed to deliver outstanding, cutting edge IT Solutions that add real value that goes beyond what is expected."
            },

            {
                title: "Our Philosophy",
                content: "Our Unique business systems analysis expertise is to enable companies to shift from legacy IT to the Future."
            },

            {
                title: "What Drives Us",
                content: "We connect the dots that will transform business for the Digital Era with Open-minded and Innovation, Excellence Execution."
            },
        ]
        ...
        var section_1 = section_1_list.map((card, index)=>{
            return(
                <div key={index} className="md:w-1/3 p-5 space-y-5 ">
                    <p className="text-blue-900 font-semibold text-xl">{card.title}</p>
        ...
        var team_mates_section = team_mates_list.map((card, index)=>{
            return(
                <div key={index} className="max-w-max space-y-1 m-4">
                    <img alt={card.name} src={card.img_path} className="w-72 h-96 "  />
                    <p className="text-center text-blue-900 font-semibold text-lg ">{card.name}</p>
                    <p className="text-center text-gray-500 text-sm ">{card.designation}</p>
                </div>
            )
        })
                    ...
                    <div className="md:flex pt-20">
                        <div className="p-5 md:w-1/2 justify-center items-center flex flex-col">
                            <div className="space-y-10">
                                <div >
                                    <p className="text-4xl text-blue-900 font-semibold">Join Our Vibrant</p>
                                    <p className="text-4xl text-blue-900 font-semibold">Professional Team</p>
                                </div>

                                <img alt="our_team_join_us" src={our_team_join_us} className="md:hidden" />


                                <p className="text-md text-gray-500">We offer like minded professionals the opportunity to develop & enhance their careers and often move ahead more rapidly than other companies. Ongoing training & mentoring provide exciting & personally satisfying careers.</p>

                                <button className="text-lg p-3 px-8 bg-gradient-to-r w-full md:w-auto text-center from-purple-400 rounded-full text-white to-cyan-600 font-semibold">Apply Now</button>
                            </div>
                        </div>

                        <img alt="our_team_join_us" src={our_team_join_us} className="w-1/2 hidden md:block" />
                    </div>
                </div>
