        ...
        var nav_section = nav_list.map((nav, index)=>{
            return(
                <div key={index} className="flex space-y-4">
                    <p className="md:text-lg text-md font-semibold text-white">{nav.cat}</p>
                    {nav.cat_list.map((sub_nav, sub_index)=>{
                        return(
                            <div key={`#${sub_index}`} className="">
                                <a href={sub_nav.nav_to} className="p-6 text-gray-200 text-2xs md:text-sm">{sub_nav.nav_name}</a>
                            </div>
                        )
                    })}
                </div>
